...
auto Compilation::codegen(const LinkedIR<SemaIR>& input_ir) -> bool
{
    ...
    codegen::trilogy::CodeGen codegen(*symbol_table.lookup_file("MAIN"),
                                              0, *storage_table, *diag_manager);

    codegen::RelocationTable reloc_table(symbol_table);
    if(!codegen.generate(input_ir, reloc_table, std::back_inserter(output)))
        return false;

    // TODO multifile generator

    // TODO relocation table fixup in the output
...
int main()
{
    const std::filesystem::path input_file = "test.sc";
    ...
    gta3sc::DiagnosticHandler diag_manager([](const gta3sc::Diagnostic& diag) {
        std::fprintf(stderr, "error: diag-%d\n",
                     static_cast<int>(diag.message));
        for(const auto& arg : diag.args)
        {
            std::fprintf(stderr, "  arg: %s\n",
                         std::get_if<std::string>(&arg)->c_str());
        }
    });
